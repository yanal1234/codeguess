...
const correctGuessesArray =guessedLetters.filter(function(prev){
  if(chipsworld.includes(prev)){
    return prev;
  }
})

const correctsound= useRef(new Audio(correctSound));
const errorsound= useRef(new Audio(errorSound));

useEffect(() => {
  const correct = correctsound.current;
  const error = errorsound.current;

  if (!correct || !error) return;
...
if(guessedLetters.length===0)
  return;
  if (chipsworld.includes(guessedLetters[guessedLetters.length-1])) {
    correct.play();
  } 
  else if (!chipsworld.includes(guessedLetters[guessedLetters.length-1])) {
    error.play();
  }

}, [guessedLetters]);

const GameMenuSelect=useRef(new Audio(gamemenuselect));

useEffect(()=>{
GameMenuSelect.current.play();
},[difficulty])
...
{showModal && gamelost && (
  ...
  <section role="status" aria-live="polite" className={classnamegame_state}>
   ...
   {difficulty&& <section role="status" aria-live="polite" className="game-state-start">
    <p>{Hint}</p>
  </section>}
...
{difficulty === null && (
 ...
 {!difficulty && !gameover&& <button onClick={obj.fun} className="back-btn">← Back to Menu</button>}
